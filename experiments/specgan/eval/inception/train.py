...
def model(x, nlogits, train=False):
  ...
  if args.infer_legacy:
    ...
    for i in xrange(0, 16384, 128):
      x_window = x[:, i:i+1024]
      x_padded = tf.pad(tensor=x_window, paddings=[[0, 0], [0, max(0, i + 1024 - 16384)]])
      x_windowed = x_padded * window
      windows.append(x_windowed)
    windows = tf.stack(windows, axis=1)
    X = tf.signal.rfft(windows)
  else:
  ...
  x = tf.compat.v1.layers.conv2d(x, 128, (5, 5), padding='same', activation=tf.nn.relu)
  x = tf.compat.v1.layers.max_pooling2d(x, (2, 2), (2, 2))
  ...
  x = tf.compat.v1.layers.dense(x, nlogits)
...
def record_to_xy(example_proto, labels):
  features = {
      'samples': tf.io.FixedLenSequenceFeature([1], tf.float32, allow_missing=True),
  ...
  wav = wav[:16384]
  wav = tf.pad(tensor=wav, paddings=[[0, 16384 - tf.shape(input=wav)[0]]])
  wav.set_shape([16384])
  ...
  label = tf.strings.reduce_join(inputs=label_chars, axis=0)
  ...
  with tf.control_dependencies([tf.compat.v1.assert_equal(nmatches, 1)]):
    return wav, label_id
...
def eval(fps, args):
  ...
  with tf.compat.v1.name_scope('eval_loader'):
  ...
  xent_mean = tf.reduce_mean(input_tensor=xent)

  preds = tf.argmax(input=logits, axis=1, output_type=tf.int32)
  acc = tf.reduce_mean(input_tensor=tf.cast(tf.equal(preds, y), tf.float32))
  step = tf.compat.v1.train.get_or_create_global_step()

  summaries = [
      tf.compat.v1.summary.scalar('acc', acc),
      tf.compat.v1.summary.scalar('xent', xent_mean)
  ]
...
def infer(args):
  ...
  x = tf.compat.v1.placeholder(tf.float32, [None, 16384], name='x')
  ...
  pred = tf.argmax(input=logits, axis=1, output_type=tf.int32, name='pred')
  ...
  saver = tf.compat.v1.train.Saver(all_vars + [global_step])
...
def train(fps, args):
  ...
  opt = tf.compat.v1.train.GradientDescentOptimizer(1e-4)
  train_op = opt.minimize(xent_mean, global_step=tf.compat.v1.train.get_or_create_global_step())

  preds = tf.argmax(input=logits, axis=1, output_type=tf.int32)
  acc = tf.reduce_mean(input_tensor=tf.cast(tf.equal(preds, y), tf.float32))

  tf.compat.v1.summary.audio('x', tf.expand_dims(x, axis=2), 16000)
  tf.compat.v1.summary.scalar('xent', xent_mean)
  tf.compat.v1.summary.scalar('acc', acc)
  tf.compat.v1.summary.histogram('xent', xent_mean)
