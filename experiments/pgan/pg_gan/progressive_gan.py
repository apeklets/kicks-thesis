...
class ProgressiveGAN(BaseGAN):
    ...
    def optimizeD(self, allLosses):
        ...
        inputLatent1, targetRandCat1 = self.buildNoiseData(batch_size, self.realLabels, skipAtts=False)
        if getattr(self.config, 'style_mixing', False):
            inputLatent2, targetRandCat2 = self.buildNoiseData(batch_size, self.realLabels, skipAtts=False)
