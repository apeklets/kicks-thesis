...
import numpy as np
import sys
sys.path.append("../../notebooks/quality")
...
class GANTrainer():
    ...
    def saveCheckpoint(self,
        ...
        # Save gen
        # postprocess = AudioPreprocessor(**self.dataConfig['transformConfig']).get_postprocessor()
        # eval_manager = StyleGEvaluationManager(self.model, n_gen=30)
        # gen_batch = eval_manager.test_random_generation()
        # audio_out = map(postprocess, gen_batch)
        # output_dir = mkdir_in_path(outDir, f"generation")
        # output_dir = mkdir_in_path(output_dir, f"s{scale}_i{iter}")
        # saveAudioBatch(audio_out,
        #            path=output_dir,
        #            basename='random', 
        #            sr=self.dataConfig["transformConfig"]["sample_rate"])
        # qual = np.mean(list(map(kick_qual, audio_out)))
        # self.quals.append(qual)
        # print(f"Quality at s{scale} i{iter}: {qual}")


    def saveQuals(self, path):
        with open(path, "wb") as f:
            pkl.dump(self.quals, f)
