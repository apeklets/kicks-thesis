...
def train_wgan(model_gen, model_dis, train_gen, valid_data, test_data,
               ...
               sample_size=20, lr=1e-4, beta_1=0.5, beta_2=0.9, latent_dim=100, epochs_per_layer=None, fading_method=None):
    ...
    if epochs_per_layer is not None:
        model_gen.setLayers(2)
        model_dis.setLayers(2)
        LOGGER.info("Training layer 2")
        resampler = Resample(44100, 44100 / 256)

        valid_data_vr = resampler(valid_data_v)
        test_data_vr = resampler(test_data_v)
        fading_idx = 0
    else:
        valid_data_vr = valid_data_v
        test_data_vr = test_data_v
    ...
    for epoch in range(num_epochs):
        ...
        if epochs_per_layer is not None:
            if epoch > 0 and epoch % epochs_per_layer == 0 and model_gen.num_layers < 6:
                model_gen.addLayer()
                model_dis.addLayer()
                fading_idx = 0
                LOGGER.info(f"Training layer {model_gen.num_layers}")
                downsample_factor = 4**(6 - model_gen.num_layers)
                resampler.new_freq = 44100 / downsample_factor
                valid_data_vr = resampler(valid_data_v)
                test_data_vr = resampler(test_data_v)
            if fading_method is not None and fading_idx <= epochs_per_layer // 2: 
                fading_const = fading_idx / (epochs_per_layer // 2)
                if fading_method == "alpha":
                    # linearly fade the new layer:
                    model_gen.setAlpha(fading_const)
                    model_dis.setAlpha(fading_const)
                    fading_idx += 1
                elif fading_method == "eta":
                    # warming up learning rate
                    minlr = 0.1*lr
                    maxlr = lr
                    lrt = minlr + fading_const*(maxlr - minlr)
                    LOGGER.info(f"Learning rate: {lrt}")
                    for opt in [optimizer_gen,optimizer_dis]:
                        opt.defaults['lr'] = lrt
                    for g in opt.param_groups:
                        g['lr'] = lrt
                else:
                    raise NotImplementedError("Layer fading method should be one of ['alpha', 'eta']")

        #with autograd.profiler.profile(
        #    enabled=(epochs_per_layer is not None and epoch > 0 and epoch % epochs_per_layer == 0),
        #    use_cuda=True,
        #    record_shapes=True,
        ...
        for batch_idx in range(batches_per_epoch):
            ...
            for iter_d in range(discriminator_updates):
                ...
                if epochs_per_layer is not None:
                    real_data_v = resampler(real_data_v)
                ...
                D_cost_valid, D_wass_valid = compute_discr_loss_terms(
                    model_dis, model_gen, valid_data_vr, noise_v, batch_size,
                    latent_dim,
                    lmbda, use_cuda, compute_grads=False)
                ...
                batch_history['discriminator'].append({
                    ...
                    'cost': D_cost_train.data.numpy(),
                    'wasserstein_cost': D_wass_train.data.numpy(),
                    'cost_validation': D_cost_valid.data.numpy(),
                    'wasserstein_cost_validation': D_wass_valid.data.numpy()
                })
        ...
        #if epochs_per_layer is not None and epoch > 0 and epoch % epochs_per_layer == 0:
        #    print(f"Profile of Layer {model_gen.num_layers}:")
        ...
        if (epoch + 1) % epochs_per_sample == 0:
            ...
            if output_dir:
                ...
                if epochs_per_layer is not None:
                    save_samples(samples[epoch+1], epoch+1, output_dir, fs=int(resampler.new_freq))
                else:
                    save_samples(samples[epoch+1], epoch+1, output_dir, fs=44100)
    ...
    final_discr_metrics = {
        ...
        'cost_validation': D_cost_valid.data.numpy(),
        'wasserstein_cost_validation': D_wass_valid.data.numpy(),
        'cost_test': D_cost_test.data.numpy(),
        'wasserstein_cost_test': D_wass_test.data.numpy(),
    }
